...
@IonicPage({
  name: 'AdminProcessDetailPage',
  segment: 'admin/process/processedit',
  defaultHistory: ['AdminPage']
})
@Component({
  selector: 'page-admin-process-detail',
  templateUrl: 'admin-process-detail.html',
})
export class AdminProcessDetailPage {
  ...
  validateInput() {
    if (this.itemInput.title == '' || this.itemInput.detail == '')
      ...
      {
        this.processFirebaseController.updateNews(this.currentChosenCatergory, this.itemInput.id, this.itemInput).then(() => {
      ...
      promiseList.push(
        this.fbservices.fbUploadFiles(media, 'news').then(
          res => {
            ...
            console.log(res)
            //saveurl to news
            this.processFirebaseController.addNewsMediaFileURLToDocumentObject(res, this.currentChosenCatergory, this.itemInput.id)
          }
        ).catch(err => {
          console.log(err)
        })
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              ...
              promiseList.push(this.processFirebaseController.setMediaArrayToDocument(this.modificationMediaList, this.currentChosenCatergory, this.itemInput.id));
              let self = this;
              Promise.all(promiseList).then(function (values) {
                self.global.dismissLoading()
