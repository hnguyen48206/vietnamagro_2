...
@IonicPage({
  ...
  segment: 'admin/products/productedit',
  defaultHistory: ['AdminPage']
})
@Component({
  selector: 'page-admin-product-detail',
...
export class AdminProductDetailPage {
  ...
  validateInput() {
    if (this.itemInput.title == '' || this.itemInput.detail == '')
      ...
      {
        this.productsFirebaseController.updateNews(this.currentChosenCatergory, this.itemInput.id, this.itemInput).then(() => {
      ...
      promiseList.push(
        this.fbservices.fbUploadFiles(media, 'news').then(
          res => {
            ...
            console.log(res)
            //saveurl to news
            this.productsFirebaseController.addNewsMediaFileURLToDocumentObject(res, this.currentChosenCatergory, this.itemInput.id)
          }
        ).catch(err => {
          console.log(err)
        })
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              ...
              promiseList.push(this.productsFirebaseController.setMediaArrayToDocument(this.modificationMediaList, this.currentChosenCatergory, this.itemInput.id));
              let self = this;
              Promise.all(promiseList).then(function (values) {
                self.global.dismissLoading()
