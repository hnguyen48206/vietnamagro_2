...
@Injectable()
export class processFirebaseController {
  ...
  createNews(detail, collectionID) {
    ...
    return new Promise(function (resolve, reject) {
      let collect = self.afs.collection('process').doc('processParentDoc').collection(collectionID)
  ...
  getAllNewsCatergoriesMetadata() {
    let self = this;
    return new Promise(function (resolve, reject) {
      let collect = self.afs.collection('process').doc('processParentDoc').collection('metadata').doc('metadata');
  ...
  getNewsOfCatergory(catergoryID) {
    let self = this;
    return new Promise(function (resolve, reject) {
      let collect = self.afs.collection('process').doc('processParentDoc').collection(catergoryID)
      let query = collect.ref
      query.get().then(
        res => {
          console.log(res)
  ...
  addNewsMediaFileURLToDocumentObject(url, catergoryID, newsID) {
    let self = this;
    return new Promise(function (resolve, reject) {
      const docRef =  self.afs.collection('process').doc('processParentDoc').collection(catergoryID).doc(newsID)
